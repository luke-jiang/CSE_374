...
int get_rand_range(int from, int to);
  ...
  for (int i = 0; i < ntrials; i++) {
    ...
    if (randNum < pctget) {    // getmem
      ...
      if (rand() % 100 < pctlarge) {
        request_size = get_rand_range(small_limit, large_limit);
      } else {
        request_size = get_rand_range(1, small_limit);
      }
      // request to getmem
      ...
      int to_free = rand() % used_blocks_size;
